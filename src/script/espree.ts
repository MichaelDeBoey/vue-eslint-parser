...
const ESLintPathPart = `eslint${path.sep}lib${path.sep}linter${path.sep}linter.js`
...
export function getEspree(): ESLintCustomParser {
    if (!espreeCache) {
        ...
        const eslintPath = Object.keys(require.cache).find(p =>
            p.includes(ESLintPathPart),
        )
        if (eslintPath) {
            try {
                espreeCache = createRequire(eslintPath)("espree")
            } catch {
                // Ignore.
            }
        }
